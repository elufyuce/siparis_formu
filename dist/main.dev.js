...
var erkekParfumleri = ["Celvin Clein", 100, "Lacoste", 120, "Axe", 30, "First Class", 50, "Giorgio Armani", 1000, "Versace", 600];
var kadinParfumleri = ["Burbery", 150, "Avon", 80, "She", 60, "Nina Ricci", 130, "Chanel", 500, "Giorgio Armani", 800]; //Girilen veriler için değişkenler

var urunTipi, urunFiyati, urunAdedi, taksitSayisi; //Çıkan sonuçlar için değişkenler

var araToplam,
    odenecekTutar,
    kargoUcreti = 19.90; //Global döngü değişkeni

var i; //Global option oluşturma değişkeni

var secenek; //Global select nesnesini tutmak için liste değişkeni 
...
function listeDoldur() {
  ...
  document.querySelectorAll('#urunListesi option').forEach(function (option) {
  ...
  console.log(urunTipi); //Elde edilen ürün tipi seçimine göre ilgili diziden değerleri alıp getirme
...
function fiyatHesapla() {
  ...
  urunAdedi = liste.options[liste.selectedIndex].value; //Ürünün fiyatı alındı.

  liste = document.getElementById("urunListesi");
  urunFiyati = liste.options[liste.selectedIndex].value; //Taksit sayısı seçimi alındı.

  liste = document.getElementById("urunTaksidi");
  taksitSayisi = liste.options[liste.selectedIndex].value; //Ürün adedine göre birim fiyattan ara toplam hesaplandı

  araToplam = urunAdedi * urunFiyati; //Taksit seçimine göre vade farkları eklendi
  ...
  document.querySelector("#txtAraToplam").value = araToplam.toFixed(2); //Ara toplam tutarına göre kargo ücreti eklenip eklenmeyeceği belirlendi ve yazdırıldı
